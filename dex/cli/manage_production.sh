...
if [[ $(which mongo) == "mongo-replicaSet not found" ]]; then
  echo "mongo-replicaSet is missing"
fi
...
show_mongo_menu(){
    ...
    echo -e "${MENU}${NUMBER} 11)${MENU} Seed Wallets (Rinkeby) ${NORMAL}"
    echo -e "${MENU}${NUMBER} 12)${MENU} Seed Wallets (Mainnet) ${NORMAL}"
...
show_contracts_menu(){
  ...
  echo -e "${MENU}${NUMBER} 3)${MENU} Show mainnet contract addresses ${NORMAL}"
  echo -e "${MENU}${NUMBER} 4)${MENU} Back ${NORMAL}"
  echo -e "${MENU}*********************************************${NORMAL}"
  read opt
